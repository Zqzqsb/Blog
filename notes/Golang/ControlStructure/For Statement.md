...
or 结构的第二种形式是没有头部的条件判断迭代（类似其它语言中的 while 循环），基本形式为：`for 条件语句 {}`。
...
想要直接退出循环体，可以使用 break 语句或 return 语句 直接返回。
...
+ break 和 continue 的用法和C++中的用法基本一致。
