...
**注意**
+ 子序列不一定是原序列中连续的一段。
+ 最长的子序列不一定以a的最后一个元素结尾,意味着答案是`f`中的最大值。
...
![队列延伸](https://alicloud-pic.oss-cn-shanghai.aliyuncs.com/BlogImg/Algorithm/linear_dp_problems/LIS_queue_extend.png)

#### 队列的更新

现在考虑一个新元素 `5` , 该元素无法得到一个更长的**LIS**，因为当前状态下最长序列的结尾元素是`6` 我们找到第一个大于`5`的位置，也就是`6`所在的位置做替换。这是因为`5`可以接续在元素`4`，同样得到一个长度为5的序列，而对于两个长度为5的上升子序列
+ `[.... ] 6`
+ `[.... ] 5`
...
对于每个位置，可能从该位置的左上方和右上方到达，选择其中累计更大的那个作为转移。
